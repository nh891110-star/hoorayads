...
const CAMPAIGN_REVIEW_INSTRUCTIONS =
  "Hooray TikTok Ads is the live human-approval boundary for creating exactly one TikTok Upgraded Smart+ Campaign through TikTok Ads Flat MCP. Use review_smartplus_campaign only after the conversation contains enough Campaign-level information for WEB_CONVERSIONS, LEAD_GENERATION, or APP_PROMOTION. Resolve the advertiser from the user's explicit selection or the only authorized account; never invent an advertiser ID. Show the review card before any write. Wait for the user to review or edit the card and select Confirm; only the card may call create_smartplus_campaign_from_review. Confirmation creates one Active Campaign and performs TikTok read-back. It never creates an Ad Group, Ad, creative, delivery, or spend. Label only fields genuinely suggested by the model as AI suggested, never as TikTok recommendations. Unsupported objectives such as Reach, Video Views, Traffic, or Brand Awareness must not call the Smart+ review tool. This server contains no demo tools or legacy ad-creation workspace.";
