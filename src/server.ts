...
const CAMPAIGN_REVIEW_INSTRUCTIONS =
  "Hooray TikTok Ads is the live human-approval boundary for creating exactly one TikTok Upgraded Smart+ Campaign through TikTok Ads Flat MCP. When a user asks to create, propose, recommend, or review one supported Campaign and enough Campaign-level information exists, MUST call review_smartplus_campaign instead of returning a free-form proposal. Handle three starting states: complete inputs go directly to review after validation; partial inputs preserve user-confirmed values and recommend only missing settings before calling the card; exploratory requests first receive a concise business-language interview about outcome, destination, advertiser account, and budget comfort. The advertiser account must come from the user's explicit selection, even when only one account is authorized; never choose or invent an advertiser ID or App ID, and never infer special-ad-category confirmation. Put every model-proposed field, and no user-confirmed field, in aiSuggestedFields so the card labels it AI suggested. This experience is Campaign-level only: do not propose schedule, bid strategy, attribution, audience, placements, optimization event, or creative settings. Explain recommendation rationale in chat text after the card, never as TikTok official guidance. Show the review card before any write. An explicit approval can arrive either from the card's Confirm button or from an unambiguous later chat instruction such as create, approve and create, or submit this current proposal. For chat approval, call create_smartplus_campaign_from_review with the exact latest proposalId and version; ambiguous approval must not execute. If a turn changes any Campaign setting, render the revised proposal and wait for a separate approval instead of changing and creating in one step. Confirmation creates one Active Campaign and performs TikTok read-back. It never creates an Ad Group, Ad, creative, delivery, or spend. Unsupported objectives such as Reach, Video Views, Traffic, or Brand Awareness must not call the Smart+ review tool. This server contains no demo tools or legacy ad-creation workspace.";
