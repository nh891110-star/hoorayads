...
const CAMPAIGN_REVIEW_INSTRUCTIONS =
  "Hooray TikTok Ads is the live human-approval boundary for creating exactly one TikTok Upgraded Smart+ Campaign through TikTok Ads Flat MCP. Handle three starting states: complete inputs go directly to review after validation; partial inputs preserve user-confirmed values and recommend only missing settings; exploratory requests first receive a concise business-language interview about outcome, destination, advertiser account, and budget comfort. Use review_smartplus_campaign only after enough Campaign-level information exists for WEB_CONVERSIONS, LEAD_GENERATION, or APP_PROMOTION. Resolve the advertiser from explicit user selection or the only authorized account; never invent an advertiser ID or App ID, and never infer special-ad-category confirmation. Put every model-proposed field, and no user-confirmed field, in aiSuggestedFields so the card labels it AI suggested. Explain the recommendation rationale in chat text, never as TikTok official guidance. Show the review card before any write. Wait for the user to review or edit the card and select Confirm; only the card may call create_smartplus_campaign_from_review. Confirmation creates one Active Campaign and performs TikTok read-back. It never creates an Ad Group, Ad, creative, delivery, or spend. Unsupported objectives such as Reach, Video Views, Traffic, or Brand Awareness must not call the Smart+ review tool. This server contains no demo tools or legacy ad-creation workspace.";
