...
2. ChatGPT 从 Hooray protected-resource metadata 发现同域 Hooray OAuth façade。
3. ChatGPT 向 Hooray DCR endpoint 注册本 connector 的官方 callback；Hooray 原样转发给 TikTok：`https://chatgpt.com/connector/oauth/{callback_id}`。
4. Hooray authorization endpoint 原样保留 client ID、官方 callback、state 和 PKCE，并跳转 TikTok。TikTok 完成授权后直接回到 ChatGPT callback；ChatGPT 再通过 Hooray token endpoint 完成 TikTok token exchange。
...
Hooray OAuth façade 是无状态的，不接收最终 callback、不保存 code/token。链路不经过 Render `/callback` 或 `/oauth/tiktok/callback`，因此 TikTok 看到的始终是 OpenAI 官方 callback，而不是 localhost、动态 IP、Cloudflare 或 Render 中转地址。
...
不要在 ChatGPT 配置中填写 TikTok developer app 的 App ID 或 App Secret。Hooray façade 和 TikTok Flat MCP 都声明 DCR public client + PKCE，`token_endpoint_auth_methods_supported: ["none"]`。

如果 ChatGPT UI 展示只读的自动发现字段，预期值如下。它们使用 Render/Hooray 同域地址是正确的：
...
- 第一次 Hooray 跳转后，Host 是 `business-api.tiktok.com`。
...
Render 中保留的旧 App ID、Secret 和 legacy callback 仅用于其他旧入口；新建 Hooray connector 不会发现或调用它们。
