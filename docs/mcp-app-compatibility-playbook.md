...
The reporting widget currently uses `v9` and preserves `v8` through `v1` as legacy aliases.
...
- Expand the rendered card and assert that a newly shipped control exists in the expected order. For reporting `v9`, `Advertiser Account` must be a searchable combobox before `Level`, `TIKTOK DIAGNOSIS` must replace `WHAT CHANGED`, and the breakdown columns must change with Campaign, Ad group, and Ad.
