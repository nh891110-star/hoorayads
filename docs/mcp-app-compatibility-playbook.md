...
The reporting widget currently uses `v8` and preserves `v7` through `v1` as legacy aliases.
...
- Expand the rendered card and assert that a newly shipped control exists in the expected order. For reporting `v8`, `Advertiser Account` must be a searchable combobox before `Level`, and `TIKTOK DIAGNOSIS` must replace `WHAT CHANGED`.
