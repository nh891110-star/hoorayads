...
**Widget:** Campaign Review `v16` with legacy aliases for cached connector versions
...
- Widget `v16` prefers ChatGPT's native app-tool API, reconciles every Edit or Confirm action against server-owned state when ChatGPT returns stale tool output, distinguishes confirmed TikTok API rejections from unknown network outcomes, and shows `Submitting…` immediately; the standard MCP Apps bridge remains the cross-host fallback.
...
1. Repeat **EDIT-01** in a fresh ChatGPT conversation that loads widget `v16`, and visually confirm the revised values remain visible.
