...
**Widget:** Campaign Review `v19` with legacy aliases for cached connector versions
...
- A signed-in ChatGPT `v18` production test changed Campaign budget from `$30/day` to `$35/day` through **Edit** > **Apply changes**. The card immediately displayed `$35/day`, while all unchanged fields remained stable.
- In the same test, **Confirm** immediately displayed `Submitting...`, called the real Campaign-only creation path, and then surfaced TikTok's official rejection: `Complete payment to continue.` The card correctly displayed `Campaign was not created`; no Campaign ID was fabricated.
- A stricter full-page reload exposed a host-cache regression in `v18`. Widget `v19` now restores the latest proposal-scoped state and short-lived action capability, then reconciles with the server. Signed-in ChatGPT retained the edited `$35/day` value after a full page reload instead of reverting to the original `$30/day` proposal.
...
- Widget `v19` uses a proposal-scoped action endpoint with a six-hour server/session TTL, while preserving the standard MCP Apps action fallback. Latest server-owned state survives host remounts and full page reloads; stale host globals cannot regress edited, submitted, failed, created, or inactive states.
...
| EDIT-01 | In a `$30/day` card, choose **Edit**, change budget to `$35/day`, then **Apply changes**. | In-card edits should create a replacement proposal without writing to TikTok. | Signed-in ChatGPT updated the visible card to `$35.00/day · Dynamic daily budget`. Objective, destination, CBO, catalog, and special-category values remained unchanged. | PASS |
| RELOAD-01 | Reload the ChatGPT page after applying the `$35/day` edit. | A full host reload must recover the latest server-owned proposal instead of replaying the original tool output. | Signed-in ChatGPT `v19` restored `$35.00/day` and kept the card actionable; it did not revert to `$30/day`. | PASS |
...
| CREATE-01 | `Use Hooray TikTok Ads Campaign Review v10. I explicitly select advertiser account Education Coaching0315. Prepare exactly one Website conversions Upgraded Smart+ Campaign named QA v18 Live Create 2026-07-20 A. Use Website destination, USD 30/day Dynamic daily budget, Campaign Budget Optimization On, Catalog not used, and I explicitly confirm no Housing, Employment, or Credit special ad category applies. Render the live Campaign Review card and wait for my confirmation. Do not use demo data and do not submit yet.` Then change budget to `$35/day` and select **Confirm**. | Verify the real write action, visible submitting state, final status reconciliation, receipt, Campaign ID, and TikTok read-back. | Confirm immediately showed `Submitting...` and `Creating one Active TikTok Campaign...`. TikTok then returned official error `40002: Complete payment to continue.` The card showed `Needs attention` and `Campaign was not created`; the edited `$35/day` value remained visible for eight seconds. No Campaign ID was returned because TikTok rejected creation before read-back. | PRODUCT FLOW PASS; ACCOUNT PAYMENT BLOCKED |
...
| Widget interaction | Proposed, Edit, Cancel, objective-specific fields, revision, create/receipt, inactive behavior, single actionable card, OAuth error, stale native submit response, direct HTTP action bridge, full host reload recovery, standard MCP status bridge, state-regression protection | PASS |
